...
//dd($request->all());
    ...
    public function fetch_teacher_data(Request $request)
    {
        ...
        $student_id=Student::where('email',$data->email) ->get('id');
        $course_id=Course_student::where('student_id',$student_id)->get('course_id');
        //dd($course_id);

             $get_teacher= DB::table('teachers')
                    ->join('course_teachers', 'teachers.id', 'course_teachers.teacher_id')
                    ->join('course_students', 'course_teachers.id', 'course_students.course_teacher_id')
        ...
        if ($course_id->count() > 0) {
            ...
            foreach ($course_id as $row) {
                $id = $row->course_id;
                $course_name=Course::where('id',$id)->get();
